...
semáforos -> gerenciador de tráfego (vias são trechos do código). Atuam como “porteiros” controlando o acesso de 'n' threads a um recurso.
mutexes   -> 'mutual exclusion' -> bloqueador de trechos de códigos. Indicados para travar (lock) acesso a um recurso comum sendo geralmente 
	     utilizados para sincronizar dois threads.
'Semáforos POSIX'
...
sem_post	-> int sem_post(sem_t *s/*semaphore_pointer*/);
		-> operação V.Incrementa semáforo.A primeira thread da fila do semáforo é desbloqueada. Incrementar/liberar. Processo não crítico.
