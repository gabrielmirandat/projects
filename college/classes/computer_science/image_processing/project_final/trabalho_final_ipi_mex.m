...
while datenum(clock) < finalTime
  ...
  skin = zeros(lin,col);
  H = zeros(lin,col);
  [skin,H] = hsi_pele_mex(lin,col,B,G,H,theta,skin,g,w,r);
  skin = uint8(skin);
  R = uint8(R);
  G = uint8(G);
  B = uint8(B);
  ...
  im6 =uint8(B);
  hair = zeros(lin,col);
  [im4,im5,im6,hair] = hsi_cabelo_mex(lin,col,hair,I,B,G,R,H,im4,im5,im6);
  imhair=cat(3,im4,im5,im6);
  
  %     Cria a quantização da pele e do cabelo;
  qtzskin = zeros(lin,col);
  qtzhair = zeros(lin,col);
  [qtzhair,qtzskin] = qtz_pele_cabelo_mex(lin,col,skin,hair,qtzskin,qtzhair);
