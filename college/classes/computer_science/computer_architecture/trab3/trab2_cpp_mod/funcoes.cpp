...
int32_t lw(uint32_t address, uint16_t kte) {
    ...
    cout << "t1 (celula de memoria) = "  << hex << t1 << endl;
    s0 = t1;                                      									 					//s0 recebe a word
    cout << "s0 (celula de memoria) = " << hex << s0 << endl;
    return s0;
}
...
void sb(uint32_t address, uint16_t kte, int8_t dado) {
    ...
    cout << "t0 (valor contido na celula de memoria) = " << hex << t0 << endl;
    t1 = dado;                         															//t1 recebe o byte a ser escrito com offset
    t1 = t1 << offset*8;
    cout << "t1 (byte a ser escrito com offset) = " << hex << t1 << endl;
    s0 = ((~(mascara_byte << (offset*8)) & t0) | t1);         									//s0 recebe o novo valor da celula
    cout << "s0 (novo valor da celula de memoria) = " << hex << s0 << endl;
    mem[celula] = s0;
}
...
void sh(uint32_t address, uint16_t kte, int16_t dado) {
    ...
    t1 = dado;                         															//t1 recebe o byte a ser escrito com offset
    t1 = t1 << offset*8;
    cout << "t1 (meia palavra a ser escrita com offset) = " << hex << t1 << endl;
...
void sw(uint32_t address, uint16_t kte, int32_t dado) {
    ...
    cout << "t0 = (novo valor da celula de memoria) = " << hex << t0 << endl;
    cout << "t1 = (novo valor da celula de memoria) = " << hex << t1 << endl;
    cout << "s0 = (novo valor da celula de memoria) = " << hex << s0 << endl;
}
