...
int8_t lb(uint32_t address, uint16_t kte) {
    ...
    printf("t1 (offset sem mascara) = %d\n",t1);
    s0 = mascara_byte & t1;                        //s0 recebe o byte com o offset com mascara
    printf("s0 (offset com mascara) = %d",s0);
    return (int8_t) s0;
}
...
void sb(uint32_t address, uint16_t kte, int8_t dado) {
    ...
    printf("t0 (valor contido na celula de memoria)= %d\n",t0);
    t1 = dado;                         //t1 recebe o byte a ser escrito com offset
    t1 = t1 << (offset*8);
    printf("t1 (byte a ser escrito com offset) = %d\n",t1);
    t0 = mascara_byte << (offset*8);
    t0 = ~t0;
    t0 = t0 & mem[celula];
    t0 = t0 | t1;
    s0 = t0;         //s0 recebe o novo valor da celula
    printf("s0 (novo valor da celula de memoria) = %d",s0);
    mem[celula] = s0;
}
