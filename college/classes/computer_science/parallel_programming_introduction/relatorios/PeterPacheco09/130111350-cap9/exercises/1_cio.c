/* Modified cio.c -- Collective functions for basic I/O operations
 *
...
int Cache_io_rank(
    ...
    } else if ((retval = Copy_attr(io_comm, io_comm, IO_KEY)) != NO_IO_ATTR) {
        /* Value cached */
        return retval;
    } else if ((retval = Copy_attr(orig_comm, io_comm, IO_KEY)) != NO_IO_ATTR) {
        /* Value cached */
        return retval;
    } 
    ...
    if ((retval = Copy_attr(orig_comm, io_comm, MPI_IO)) != NO_IO_ATTR) {
        /* Value cached */
#ifdef DEBUG
       printf("Process %d > Copied attribute from orig to io\n", my_rank);
#endif
        return retval;
    } else if ((retval = Copy_attr(io_comm, io_comm, MPI_IO)) != NO_IO_ATTR) {
        /* Value cached */
#ifdef DEBUG
