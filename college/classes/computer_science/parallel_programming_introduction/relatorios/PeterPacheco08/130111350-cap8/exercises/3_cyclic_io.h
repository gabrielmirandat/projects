...
typedef struct 
{
    ...
    float     entries[ENTRIES_MAX]; /* Elements of the array   */
    #define Entries(array)        ((array)->entries)
    #define Entry(array,i)        ((array)->entries[i])
    float     local_entries[LOCAL_ENTRIES_MAX]; /* Local elements of the array */
    ...
    int       stride;        /* Number of elements between two successive local entries */
