/* Modified get_data.c -- Parallel Trapezoidal Rule, uses basic Get_data function for 
 *     input.
 *
 * Input: - read in by process 0 -
...
/* Funtion definition */
void Get_data(float* a_ptr, float* b_ptr, int* n_ptr, int* func, int my_rank, int p);

/* Calculate local integral  */
float Trap(float local_a, float local_b, int local_n, int func, float h);   
...
int main(int argc, char** argv) {
    ...
    integral = Trap(local_a, local_b, local_n, func, h);
...
void Get_data(
    ...
    if (my_rank == 0){
        ...
        printf("Selecione a funcao a ser usada:\n");
        printf("1. -x\n");
        printf("2. 3x+5\n");
        printf("3. x^2\n");
        scanf("%d", func_ptr);
        ...
        for (dest = 1; dest < p; dest++){
            ...
            tag = 3;
            MPI_Send(func_ptr, 1, MPI_INT, dest, tag, MPI_COMM_WORLD);
        }
    } else {
        tag = 0;
        ...
        tag = 3;
        MPI_Recv(func_ptr, 1, MPI_INT, source, tag, MPI_COMM_WORLD, &status);
    }
} /* Get_data */
...
float Trap(
    ...
    float f(float x, int local_func); /* function we're integrating */

    integral = (f(local_a, local_func) + f(local_b, local_func) + f((local_b+local_a)/2, local_func))/3.0; 
    x = local_a; 
    for (i = 1; i <= local_n-1; i++) { 
        x = x + h; 
        integral = integral + f(x, local_func); 
    } 
    integral = integral*h; 
    return integral;
} /*  Trap  */
