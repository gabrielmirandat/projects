...
#Aquisição de dados
    ...
    #
        ...
        #Botão de saída
        Button(t, text='Sair', command=self.stap).grid(row=5, column=1, sticky=W, pady=4)
...
#Imprime o arquivo de dados
    ...
    #
        ...
        for json in file_:
            ...
            dados_arquivo = dados_arquivo + file_.readline()
...
#############################################################################################################
    ...
    #
        ...
        if e2.get() is not None:
            segredo_consumidor = e2.get()
        if e3.get() is not None:
            chave_tokenAcesso = e3.get()
        if e4.get() is not None:
            segredo_tokenAcesso = e4.get()
...
#Run da thread de interface gráfica
    ...
    #
        ...
        token_config = Button(frame, text = "Configurar Tokens",command = self.entryWidget, width = 20)
