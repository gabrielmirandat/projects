...
GLuint sphere(float radius, int slices, int stacks) {
  ...
  for(i=0;i<NumVertices;i++){
    aux = (aux + (1.0/(GLfloat)NumVertices));
    if(i<NumVertices/5 && i>NumVertices/7 && i%13==0){ colors[i] = vec4(0.7,0.4,0.1,1.0);}
    else if(i<NumVertices && i>NumVertices/2 && i%45==0){ colors[i] = vec4(0.7,0.4,0.1,1.0);}
    else if(i<NumVertices/2 && i>NumVertices/5 && i%30==0){ colors[i] = vec4(0.7,0.4,0.1,1.0);}
    else colors[i] = vec4(0.0,0.1,aux,1.0);
  }
