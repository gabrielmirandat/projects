...
/* Basciamente, o carregamento consistem em 3 atividades básicas. Para carregar um tipo, a JVM tem que:
...
Dado o nome do tipo, gerar um stream de binários que represente o mesmo
...
*/
	...
	if(cf->fields_count){
		...
		for(uint16_t i = 0; i < cf->fields_count; i++){
			...
			if(! DescritorCampo(0, ((*cd)->field_data + i)->field_descriptor)){    /* MUDANÇAS*/
	...
	if(cf->methods_count){
		...
		for(uint16_t i = 0; i < cf->methods_count; i++){
			...
			if(! DescritorMetodo(((*cd)->method_data + i)->method_descriptor, 1)){       /*Mudanças*/
				puts("VerifyError: method descriptor");
				exitJVM(jvm);
				exit(EXIT_FAILURE);
			}
...
*/
	ArquivoClassverificador(cd->classfile);        /* Mudanças */
