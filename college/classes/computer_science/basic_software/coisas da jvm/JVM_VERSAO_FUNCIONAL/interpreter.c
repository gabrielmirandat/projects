...
/*https://docs.oracle.com/javase/specs/jvms/se7/html/jvms-6.html#jvms-6.5.putfield*/
	...
	// CONTROLE DE ACESSO
	uint8_t	* backupPC = thread->program_counter;
	METHOD_AREA	* field_class = getClass(cp_class_name, jvm);
	if(!field_class){// se a classe do field não foi carregada
		...
		loadClass(string, &field_class, method->METHOD_AREA, jvm);
		free(string);
		...
		puts("\n=======================");
		printf("Resume\t");
		PrintConstantUtf8((method->METHOD_AREA)->class_name, stdout);
		...
		if(field_class != method->METHOD_AREA){// Se o Field não for da mesma classe
			// verifica se a classe do field é acessível pelo método corrente
			if(!(field_class->modifiers & ACC_PUBLIC) &
			(field_class->class_loader_reference != (method->METHOD_AREA)->class_loader_reference)){
	...
	switch(* thread->program_counter){
		...
		case	getfield:
			...
			if((var->field_reference)->modifiers & ACC_PROTECTED){ // SE O FIELD É PROTECTED
				if(isSuperClass(field_class, method->METHOD_AREA, jvm)){// se a classe do field é superclasse da classe corrente.
					if((method->METHOD_AREA)->class_loader_reference != field_class->class_loader_reference){
						if(!(objectref->method_area_reference == method->METHOD_AREA) &&
						 !isSuperClass(method->METHOD_AREA, objectref->method_area_reference, jvm)){
...
/*	https://docs.oracle.com/javase/specs/jvms/se7/html/jvms-5.html#jvms-5.4.4*/
	uint8_t	* backupPC = thread->program_counter;
	METHOD_AREA	* method_class = getClass(cp_class_name, jvm);
	if(!method_class){// se a classe do método não foi carregada, mesmo package
		...
		loadClass(string, &method_class, method->METHOD_AREA, jvm);
		free(string);
	...
	if(!(invoked_method->modifiers & ACC_PUBLIC)){// SE O MÉTODO NÃO É PUBLICO
		if(invoked_method->modifiers & ACC_PROTECTED){ // SE O MÉTODO É PROTECTED
			if(method->METHOD_AREA != method_class){ // Se a classe do método invokado é diferente da classe método corrente
				if(!isSuperClass(method->METHOD_AREA, method_class, jvm)){
				// Se a classe do método invokado é não é subclasse da classe método corrente
					if((method->METHOD_AREA)->class_loader_reference != method_class->class_loader_reference){
...
/*				strcat((char *) (string_buffer->instance_variables)->value.u.InstanceReference.reference, append_string);*/
	...
	switch(* thread->program_counter){// PARA TESTAR O HELLOWORLD
		case	invokevirtual:
			...
			METHOD_AREA	* class_objectref = objectref->method_area_reference;
			if((invoked_method->modifiers & ACC_PROTECTED)){
				METHOD_AREA	* super_class = getSuperClass((method->METHOD_AREA)->classfile, jvm);
				...
				if(isSuperClass && ((invoked_method->METHOD_AREA)->class_loader_reference !=
							 (method->METHOD_AREA)->class_loader_reference )){
					if(class_objectref != method->METHOD_AREA){
						isSuperClass = false;
						super_class = getSuperClass(class_objectref->classfile, jvm);
						while(super_class && !isSuperClass){
							if(method->METHOD_AREA == super_class){
								isSuperClass = true;
							}
							else{
								super_class = getSuperClass(super_class->classfile, jvm);
			...
			while(super_class && !findMethod){
				if((invoked_method = getMethod(method_name, method_descriptor, super_class, jvm))){
					if(!(invoked_method->modifiers & ACC_STATIC)){
						...
						executeMethod(method_name, method_descriptor, invoked_method->METHOD_AREA,
		...
		case	invokespecial:;
			...
			if(((method->METHOD_AREA)->modifiers & ACC_SUPER) && (isSuperClass) && strcmp(method_name, "<init>")){
				bool	findMethod = false;
				super_class = getSuperClass((method->METHOD_AREA)->classfile, jvm);
				while(super_class && !findMethod){
					if((invoked_method = getMethod(method_name, method_descriptor, super_class, jvm))){
						if(!(invoked_method->modifiers & ACC_STATIC)){
							if(invoked_method->modifiers & ACC_ABSTRACT){
		...
		case	invokeinterface:
			...
			class_objectref = objectref->method_area_reference;
...
/*https://docs.oracle.com/javase/specs/jvms/se7/html/jvms-6.html#jvms-6.5.multianewarray*/
	...
	switch(* thread->program_counter){// PARA TESTAR O HELLOWORLD
		case	new:
			...
			// CONTROLE DE ACESSO
			backupPC = thread->program_counter;
			METHOD_AREA	* object_class = getClass(cp_class_name, jvm);
			if(!object_class){// se a classe do objeto não foi carregada

				char	* string = malloc((strlen(class_name) + 7) * sizeof(CHAR));
				strcpy(string, class_name);
				strcat(string, ".class");
				loadClass(string, &object_class, method->METHOD_AREA, jvm);
				free(string);
				...
				if(object_class != method->METHOD_AREA){// Se o objeto não for da mesma classe do método
					...
					(object_class->class_loader_reference != (method->METHOD_AREA)->class_loader_reference)){
						puts("IllegalAccessError: acesso indevido à classe ou interface.");
						exitJVM(jvm);
						exit(EXIT_FAILURE);
					}
			...
			newObject = (OBJECT *) malloc(sizeof(OBJECT));
			newObject->method_area_reference = object_class;
			newObject->next = (jvm->heap_jvm)->objects;
			(jvm->heap_jvm)->objects = newObject;

			newObject->instance_variables = NULL;
			METHOD_AREA	* super_class = object_class;
...
/*			}*/
		...
		case	anewarray:
			...
			// CONTROLE DE ACESSO
			backupPC = thread->program_counter;
			METHOD_AREA	* array_class = getClass(cp_class_name, jvm);
			if(!array_class){// se a classe do array não foi carregada
				...
				loadClass(string, &array_class, method->METHOD_AREA, jvm);
				free(string);
				...
				if(array_class != method->METHOD_AREA){// Se o array não for da mesma classe do método
					...
					(array_class->class_loader_reference != (method->METHOD_AREA)->class_loader_reference)){
						puts("IllegalAccessError: acesso indevido à classe ou interface.");
						exitJVM(jvm);
						exit(EXIT_FAILURE);
					}
			...
			new_array->method_area_reference = array_class;
		...
		case	multianewarray:
			...
			if(!array_class){// se a classe do array não foi carregada
				if(class_name[0] == '['){
					array_class = (METHOD_AREA *) malloc(sizeof(METHOD_AREA));
					array_class->class_name = cp_class_name;
					array_class->classfile = NULL;
					array_class->modifiers = ACC_PUBLIC;
					array_class->class_loader_reference = method->METHOD_AREA;
					...
					array_class->next = jvm->method_area_jvm;
			...
			arrayref->method_area_reference = array_class;
	...
	if(i < dimensions){
		if(count_values[i] != 0){
			...
			for(int32_t j = 0; j < arrayref->count; j++){
				...
				(arrayref->entry[j].u.ArrayReference.reference)->method_area_reference = arrayref->method_area_reference;
