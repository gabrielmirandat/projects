...
void startJVM(char * nomeDoArquivoDeClasse, int num_args, char ** args)
{
	...
	METHOD_AREA * dados_classe_principal;
...
// função loadClass
void loadClass(char * nomeDoArquivoDeClasse, METHOD_AREA ** cd, METHOD_AREA * carregadorClasse, JVM * jvm)
{
...
*/
	...
	(*cd) = (METHOD_AREA *) malloc(sizeof(METHOD_AREA));
	...
	if(cf->methods_count){
		...
		for(uint16_t i = 0; i < cf->methods_count; i++){
			...
			((*cd)->method_data + i)->METHOD_AREA = (*cd);
...
/*==========================================*/
// função linkClass
void	linkClass	(METHOD_AREA * cd, JVM * jvm){
	verifyLink(cd, jvm);
	prepareLink(cd, jvm);
...
/*==========================================*/
// função verifyLink
void	verifyLink(METHOD_AREA * cd, JVM * jvm){
/*	A IMPLEMENTAR
...
/*==========================================*/
// função prepareLink
void	prepareLink(METHOD_AREA * cd, JVM * jvm){
/*
...
/*==========================================*/
// função getClassName
char	*	getClassName(METHOD_AREA * cd){
...
METHOD_AREA	* getSuperClass(ClassFile * cf, JVM * jvm){
	if(cf->super_class){
		...
		if(strcmp(super_name, "java/lang/Object")){
			METHOD_AREA	* cd = jvm->method_area_jvm;
...
bool		isSuperClass(METHOD_AREA * super_cd, METHOD_AREA * cd, JVM * jvm){
	METHOD_AREA	* super = cd;
	while(super){
...
/*==========================================*/
// funcao getClass
METHOD_AREA	* getClass(constant_pool_info * cp_class_name, JVM * jvm){
...
VARIABLE	* getClassVariable(constant_pool_info * cp_field_name, METHOD_AREA * field_class){
...
void	initializeClass(METHOD_AREA * cd, JVM * jvm, THREAD * thread){
/*	A IMPLEMENTAR
...
/*==========================================*/
// função getMethod
METHOD_DATA	* getMethod(char * method_name, char * method_descriptor, METHOD_AREA * cd, JVM * jvm){
	char	* name;
	char	* descriptor;
	
	METHOD_AREA * super_class = cd;
	while(super_class){
...
attribute_info	* getCodeAttribute(METHOD_DATA * method, METHOD_AREA * cd){
...
/*==========================================*/
// função execute
void	executeMethod(char * method_name, char * method_descriptor, METHOD_AREA * cd, JVM * jvm, THREAD * thread, void * this, uint16_t nargs, uint32_t * args){
...
/*==========================================*/
// função unloadClass
void	unloadClass(METHOD_AREA * cd, JVM * jvm){
/*	A IMPLEMENTAR
...
void		freeMethodArea(METHOD_AREA * method_area_jvm){
	METHOD_AREA	* aux = method_area_jvm;
	while(aux){
...
void		freeClassData(METHOD_AREA * METHOD_AREA){
	if(METHOD_AREA->classfile){
		if((METHOD_AREA->classfile)->fields_count){
			free(METHOD_AREA->field_data);
		}
		if((METHOD_AREA->classfile)->methods_count){
			free(METHOD_AREA->method_data);
		}
	
		freeClassFile(METHOD_AREA->classfile);
	}
	
	if(METHOD_AREA->class_variables){
		freeVariables(METHOD_AREA->class_variables);
	}
