...
bool isFieldDescriptor(constant_pool_info *cp, uint16_t index) {
...
bool isMethodDescriptor(constant_pool_info *cp, uint16_t index) {
	uint16_t length = cp->u.Utf8.length;
	uint8_t *bytes = cp->u.Utf8.bytes + index;
...
void verifyConstantPool(ClassFile *cf) {
	...
	for (uint16_t i = 0; i < (cf->constant_pool_count - 1); i++) {
		...
		switch (cp->tag) {
			case CONSTANT_Class:
				if ((cf->constant_pool + cp->u.Class.name_index - 1)->tag !=
					CONSTANT_Utf8) {
			...
			case CONSTANT_InterfaceMethodref:
				if ((cf->constant_pool + cp->u.Ref.name_index - 1)->tag !=
					CONSTANT_Class) {
					puts("VerifyError: invalid ref class index");
					exit(EXIT_FAILURE);
				}
				if ((cf->constant_pool + cp->u.Ref.name_and_type_index -
				...
				if (cp->tag != CONSTANT_Fieldref) {
					...
					cp_aux =
						cf->constant_pool + cp->u.Ref.name_and_type_index - 1;
			...
			case CONSTANT_String:
				if ((cf->constant_pool + cp->u.String.string_index -
					 1)->tag != CONSTANT_Utf8) {
			...
			case CONSTANT_NameAndType:
				if ((cf->constant_pool + cp->u.NameAndType.name_index -
					 1)->tag != CONSTANT_Utf8) {
					puts("VerifyError: invalid NameAndType name_index");
					exit(EXIT_FAILURE);
				}
				constant_pool_info *cp_aux;
				if ((cp_aux = cf->constant_pool +
...
void verifyClassfile(ClassFile *cf) {
	...
	for (uint16_t i = 0; i < cf->fields_count; i++) {
		...
		uint16_t attributes_count = (cf->fields + i)->attributes_count;
	...
	for (uint16_t i = 0; i < cf->methods_count; i++) {
		...
		uint16_t attributes_count = (cf->methods + i)->attributes_count;
		for (uint16_t j = 0; j < attributes_count; j++) {
			cp_aux = cf->constant_pool +
		...
		switch (getAttributeType(cf->attributes + i, cf)) {
			...
			case INNER_CLASSES:;
				...
				for (uint16_t j = 0; j < number_of_classes; j++) {
					...
					cp_aux =
						cf->constant_pool + class->inner_class_info_index - 1;
					if (cp_aux->tag != CONSTANT_Class) {
						puts("VerifyError: invalid inner_class_info_index");
						exit(EXIT_FAILURE);
					}
					cp_aux =
						cf->constant_pool + class->outer_class_info_index - 1;
					if (cp_aux->tag != CONSTANT_Class) {
...
void verifyOverrideMethodFinal(ClassFile *cf, JVM *jvm) {
	...
	constant_pool_info *cp_aux = NULL;
	for (uint16_t i = 0; i < cf->methods_count; i++) {
		...
		METHOD_AREA *cd_super = cd_super = getSuperClass(cf, jvm);
		while (cd_super) {
			for (uint16_t i = 0; i < (cd_super->classfile)->methods_count;
				 i++) {
				cp_aux = ((cd_super->classfile)->methods + i)->name_index +
				...
				if (!strcmp(method_name, super_method_name)) {
					if (((cd_super->classfile)->methods + i)->access_flags ==
						ACC_FINAL) {
