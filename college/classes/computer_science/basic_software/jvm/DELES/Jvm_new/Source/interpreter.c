	...
	case ldc2_w:
		...
		switch(cp->tag)	{
			...
			case (CONSTANTE_String):
				cp_aux = (thread->jvm_stack)->current_constant_pool + cp->u.String.indicador_string - 1;
				...
				object_string->prox = (jvm->heap_jvm)->objects;
				...
				(object_string->instance_variables)->prox = NULL;
				pushOperand((uint32_t) object_string, thread->jvm_stack);
				break;
			case (CONSTANTE_Long):
	...
	switch(*thread->program_counter){
		...
		case faload:
			*value = (arrayref->entry)[index].u.Float.float_;
			break;
		case aaload:
			*value = (uint32_t) (arrayref->entry)[index].u.InstanceReference.reference;
			break;
		case baload:
...
/*https://docs.oracle.com/javase/specs/jvms/se7/html/jvms-6.html#jvms-6.5.ret*/
	...
	switch(* thread->program_counter){
		case	goto_:
			...
			break;
        case	jsr:
            pushOperand((uint32_t)(thread->program_counter += 3), thread->jvm_stack);
            thread->program_counter += branch;
			break;
        case	ret:{
            thread->program_counter++;
...
/*https://docs.oracle.com/javase/specs/jvms/se7/html/jvms-6.html#jvms-6.5.putfield*/
	...
	constPoolInf	* cp_aux = (thread->jvm_stack)->current_constant_pool;
	cp_aux = cp_aux + index - 1; // falta verificar se o indice está nos limites da constant pool
	cp_aux = (thread->jvm_stack)->current_constant_pool + cp_aux->u.Ref.indicador_nome - 1;
	constPoolInf	* cp_class_name = (thread->jvm_stack)->current_constant_pool + cp_aux->u.Class.indicador_nome - 1;
	...
	cp_aux = (thread->jvm_stack)->current_constant_pool + cp_aux->u.Ref.indicador_nome_e_tipo - 1;
	constPoolInf	* cp_field_name = (thread->jvm_stack)->current_constant_pool + cp_aux->u.NameAndType.indicador_nome - 1;
	char	* field_name = (char *) cp_field_name->u.Utf8.bytes;
	field_name[cp_field_name->u.Utf8.length] = '\0';

		// descritor do field
	constPoolInf * cp_field_descriptor = (thread->jvm_stack)->current_constant_pool + cp_aux->u.NameAndType.descriptor_index - 1;
	...
	// CONTROLE DE ACESSO
	uint8_t	* backupPC = thread->program_counter;
	DADOS_CLASSE	* field_class = getClass(cp_class_name, jvm);
	if(!field_class){// se a classe do field não foi carregada

		

		char	* string = malloc((strlen(nomeClasse) + 7) * sizeof(CHAR));
		strcpy(string, nomeClasse);
		strcat(string, ".class");


		carregarClasse_01(string, &field_class, method->DADOS_CLASSE, jvm);
		free(string);
		ligadorDeClasse(field_class, jvm);
		...
		puts("\n=======================");
		printf("Resume\t");
		PrintConstantUtf8((method->DADOS_CLASSE)->nomeClasse, stdout);
		...
		if(field_class != method->DADOS_CLASSE){// Se o Field não for da mesma classe
			// verifica se a classe do field é acessível pelo método corrente
			if(!(field_class->modifiers & ACC_PUBLIC) &
			(field_class->referencia_carregadorClasse != (method->DADOS_CLASSE)->referencia_carregadorClasse)){
	...
	switch(* thread->program_counter){
		case	getstatic:;
			...
			switch((var->value).type){
				case	BYTE:
					pushOperand((int32_t) (var->value).u.Byte.byte, thread->jvm_stack);
					break;
				case	CHAR:
					pushOperand((uint32_t) (var->value).u.Char.char_, thread->jvm_stack);
					break;
				case	FLOAT:
					pushOperand((uint32_t) (var->value).u.Float.float_, thread->jvm_stack);
					break;
				case	INT:
					pushOperand((int32_t) (var->value).u.Integer.integer, thread->jvm_stack);
					break;
				case	REF_INST:
					pushOperand((uint32_t) (var->value).u.ArrayReference.reference, thread->jvm_stack);
					break;
				case	SHORT:
					pushOperand((int32_t) (var->value).u.Short.short_, thread->jvm_stack);
					break;
				case	BOOLEAN:
					pushOperand((uint32_t) (var->value).u.Boolean.boolean, thread->jvm_stack);
					break;
				case	REF_ARRAY:
					pushOperand((uint32_t) (var->value).u.InstanceReference.reference, thread->jvm_stack);
					break;
				case	DOUBLE:
				case	LONG:;
		...
		case	getfield:
			...
			if((var->field_reference)->modifiers & ACC_PROTECTED){ // SE O FIELD É PROTECTED
				if(isSuperClass(field_class, method->DADOS_CLASSE, jvm)){// se a classe do field é superclasse da classe corrente.
					if((method->DADOS_CLASSE)->referencia_carregadorClasse != field_class->referencia_carregadorClasse){
						if(!(objectref->class_data_reference == method->DADOS_CLASSE) &&
						 !isSuperClass(method->DADOS_CLASSE, objectref->class_data_reference, jvm)){
...
/*	https://docs.oracle.com/javase/specs/jvms/se7/html/jvms-5.html#jvms-5.4.3.3*/
	...
	constPoolInf	* cp = (thread->jvm_stack)->current_constant_pool;
	constPoolInf * cp_method_ref = cp + index - 1; // falta verificar se o indice está nos limites da constant pool
	constPoolInf * cp_class = cp + cp_method_ref->u.Ref.indicador_nome - 1;
	constPoolInf	* cp_class_name = cp + cp_class->u.Class.indicador_nome - 1;
	char	* nomeClasse = (char *) cp_class_name->u.Utf8.bytes;
	nomeClasse[cp_class_name->u.Utf8.length] = '\0';

	// nome do método
	constPoolInf	* cp_name_and_type = cp + cp_method_ref->u.Ref.indicador_nome_e_tipo - 1;
	constPoolInf * cp_method_name = cp + cp_name_and_type->u.NameAndType.indicador_nome - 1;
	...
	// descritor do método
	constPoolInf	* cp_method_descriptor = cp + cp_name_and_type->u.NameAndType.descriptor_index - 1;
...
/*	https://docs.oracle.com/javase/specs/jvms/se7/html/jvms-5.html#jvms-5.4.4*/
	uint8_t	* backupPC = thread->program_counter;
	DADOS_CLASSE	* method_class = getClass(cp_class_name, jvm);
	if(!method_class){// se a classe do método não foi carregada, mesmo package
		...
		char	* string = malloc((strlen(nomeClasse) + 7) * sizeof(CHAR)); // FREE OK
		strcpy(string, nomeClasse);
		strcat(string, ".class");

		carregarClasse_01(string, &method_class, method->DADOS_CLASSE, jvm);
		free(string);
		ligadorDeClasse(method_class, jvm);
	...
	if(!(invoked_method->modifiers & ACC_PUBLIC)){// SE O MÉTODO NÃO É PUBLICO
		if(invoked_method->modifiers & ACC_PROTECTED){ // SE O MÉTODO É PROTECTED
			if(method->DADOS_CLASSE != method_class){ // Se a classe do método invokado é diferente da classe método corrente
				if(!isSuperClass(method->DADOS_CLASSE, method_class, jvm)){
				// Se a classe do método invokado é não é subclasse da classe método corrente
					if((method->DADOS_CLASSE)->referencia_carregadorClasse != method_class->referencia_carregadorClasse){
		...
		if(!strcmp(nomeClasse, "java/io/PrintStream")){
			bool	ln = false;
...
/*						printf("%s", (char *) popOperand(thread->jvm_stack));*/
						OBJECT	*	object_string = (OBJECT *) popOperand(thread->jvm_stack);
		...
		else if(!strcmp(nomeClasse, "java/lang/StringBuffer")){
			if(!strcmp(method_name, "append")){
				...
				switch(method_descriptor[1]){
					...
					case	DOUBLE:;
						...
						break;
					case	REF_INST:
						append_string = (char *) (((OBJECT *) popOperand(thread->jvm_stack))->instance_variables)->value.u.InstanceReference.reference;
						break;
				}
...
/*				strcat((char *) (string_buffer->instance_variables)->value.u.InstanceReference.reference, append_string);*/
				pushOperand((uint32_t) string_buffer, thread->jvm_stack);
		...
		else if(!strcmp(nomeClasse, "java/lang/String")){
	...
	switch(* thread->program_counter){// PARA TESTAR O HELLOWORLD
		case	invokevirtual:
			...
			DADOS_CLASSE	* class_objectref = objectref->class_data_reference;
			if((invoked_method->modifiers & ACC_PROTECTED)){
				DADOS_CLASSE	* super_class = getSuperClass((method->DADOS_CLASSE)->classfile, jvm);
				...
				if(isSuperClass && ((invoked_method->DADOS_CLASSE)->referencia_carregadorClasse !=
							 (method->DADOS_CLASSE)->referencia_carregadorClasse )){
					if(class_objectref != method->DADOS_CLASSE){
						isSuperClass = false;
						super_class = getSuperClass(class_objectref->classfile, jvm);
						while(super_class && !isSuperClass){
							if(method->DADOS_CLASSE == super_class){
								isSuperClass = true;
							}
							else{
								super_class = getSuperClass(super_class->classfile, jvm);
			...
			while(super_class && !findMethod){
				if((invoked_method = getMethod(method_name, method_descriptor, super_class, jvm))){
					if(!(invoked_method->modifiers & ACC_STATIC)){
						...
						executeMethod(method_name, method_descriptor, invoked_method->DADOS_CLASSE,
		...
		case	invokespecial:;
			...
			if(((method->DADOS_CLASSE)->modifiers & FLAG_SUPER) && (isSuperClass) && strcmp(method_name, "<init>")){
				bool	findMethod = false;
				super_class = getSuperClass((method->DADOS_CLASSE)->classfile, jvm);
				while(super_class && !findMethod){
					if((invoked_method = getMethod(method_name, method_descriptor, super_class, jvm))){
						if(!(invoked_method->modifiers & ACC_STATIC)){
							if(invoked_method->modifiers & ACC_ABSTRACT){
		...
		case	invokeinterface:
			if(cp_method_ref->tag != CONSTANTEe_Interface_referencia){
...
/*https://docs.oracle.com/javase/specs/jvms/se7/html/jvms-6.html#jvms-6.5.multianewarray*/
	...
	uint8_t	indexbyte2;
	uint16_t	index;
	constPoolInf	* cp_class;
	constPoolInf	* cp;
	constPoolInf	* cp_class_name;
	char	* nomeClasse;
	ARRAY *	new_array;
	uint8_t	* backupPC;
	ARRAY	* arrayref;
	...
	switch(* thread->program_counter){// PARA TESTAR O HELLOWORLD
		case	new:
			...
			if(cp_class->tag != Classe_const){
				puts("InstantiationError: Referência inválida para classe do objeto");
				jvmExit(jvm);
				exit(EXIT_FAILURE);
			}

			cp_class_name = cp + cp_class->u.Class.indicador_nome - 1;

			nomeClasse = (char *) cp_class_name->u.Utf8.bytes;
			nomeClasse[cp_class_name->u.Utf8.length] = '\0';

			OBJECT	*	newObject;
			if(!strcmp(nomeClasse, "java/lang/StringBuffer") || !strcmp(nomeClasse, "java/lang/String")){
				char * string = (char *) malloc(sizeof(char));
				string[0] = '\0';
				newObject = (OBJECT *)  malloc(sizeof(OBJECT));
				newObject->prox = (jvm->heap_jvm)->objects;
				...
				pushOperand((uint32_t) newObject, thread->jvm_stack);
				thread->program_counter += 3;
				return;
			}
			...
			// CONTROLE DE ACESSO
			backupPC = thread->program_counter;
			DADOS_CLASSE	* object_class = getClass(cp_class_name, jvm);
			if(!object_class){// se a classe do objeto não foi carregada

				char	* string = malloc((strlen(nomeClasse) + 7) * sizeof(CHAR));
				strcpy(string, nomeClasse);
				strcat(string, ".class");
				carregarClasse_01(string, &object_class, method->DADOS_CLASSE, jvm);
				free(string);
				ligadorDeClasse(object_class, jvm);
				classInitialization(object_class, jvm, thread);

				thread->program_counter = backupPC;
			}
			else{
				if(object_class != method->DADOS_CLASSE){// Se o objeto não for da mesma classe do método
					...
					(object_class->referencia_carregadorClasse != (method->DADOS_CLASSE)->referencia_carregadorClasse)){
						puts("IllegalAccessError: acesso indevido à classe ou interface.");
						jvmExit(jvm);
						exit(EXIT_FAILURE);
					}
			...
			DADOS_CLASSE	* super_class = object_class;
			// CRIA INSTANCE_VARIABLES
			while(super_class){
				if((super_class->classfile)->conts_campos){
					for(uint16_t i = 0; i < (super_class->classfile)->conts_campos; i++){
...
/*			}*/
		...
		case	anewarray:
			...
			// CONTROLE DE ACESSO
			backupPC = thread->program_counter;
			DADOS_CLASSE	* array_class = getClass(cp_class_name, jvm);
			if(!array_class){// se a classe do array não foi carregada
				...
				carregarClasse_01(string, &array_class, method->DADOS_CLASSE, jvm);
				free(string);
				ligadorDeClasse(array_class, jvm);
				classInitialization(array_class, jvm, thread);

				thread->program_counter = backupPC;
			}
			else{
				if(array_class != method->DADOS_CLASSE){// Se o array não for da mesma classe do método
					...
					(array_class->referencia_carregadorClasse != (method->DADOS_CLASSE)->referencia_carregadorClasse)){
						puts("IllegalAccessError: acesso indevido à classe ou interface.");
						jvmExit(jvm);
						exit(EXIT_FAILURE);
					}
			...
			if(!strcmp(nomeClasse, "java/lang/String")){
			...
			new_array->prox = (jvm->heap_jvm)->arrays;
			(jvm->heap_jvm)->arrays = new_array;

			pushOperand((uint32_t) new_array, thread->jvm_stack);
		...
		case	multianewarray:
			...
			if(!array_class){// se a classe do array não foi carregada
				if(nomeClasse[0] == '['){
					array_class = (DADOS_CLASSE *) malloc(sizeof(DADOS_CLASSE));
					array_class->nomeClasse = cp_class_name;
					array_class->classfile = NULL;
					array_class->modifiers = ACC_PUBLIC;
					array_class->referencia_carregadorClasse = method->DADOS_CLASSE;
					array_class->runtime_constant_pool = NULL;
					array_class->field_data = NULL;
					array_class->method_data = NULL;
					array_class->variaveisClasse = NULL;
					array_class->prox = jvm->method_area_jvm;
					jvm->method_area_jvm = array_class;
				}
				else{
					char	* string = malloc((strlen(nomeClasse) + 7) * sizeof(CHAR));
					strcpy(string, nomeClasse);
			...
			arrayref->prox = (jvm->heap_jvm)->arrays;
			...
			pushOperand((uint32_t) arrayref, thread->jvm_stack);
	...
	if(i < dimensions){
		if(count_values[i] != 0){
			...
			for(int32_t j = 0; j < arrayref->count; j++){
				...
				(arrayref->entry[j].u.ArrayReference.reference)->prox = (jvm->heap_jvm)->arrays;
...
/*			*/
/*			constPoolInf	* cp = (thread->jvm_stack)->current_constant_pool;*/
/*			constPoolInf	* cp_class = cp + index - 1;*/
/*			if(cp_class->tag != Classe_const){*/
...
/*https://docs.oracle.com/javase/specs/jvms/se7/html/jvms-6.html#jvms-6.5.jsr_w*/
	...
	switch(* thread->program_counter){
		case	goto_w:
			...
			break;
        case	jsr_w:
            pushOperand((uint32_t)(thread->program_counter += 5), thread->jvm_stack);
            thread->program_counter += branch;
			break;
	}
}
