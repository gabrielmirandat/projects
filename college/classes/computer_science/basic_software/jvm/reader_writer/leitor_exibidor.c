...
// libera a memoria que fora alocada para fields
...
// libera a memoria que fora alocada para methods
...
// libera a memoria que fora alocada para attributes
void freeAttribute(attribute_info * attr, ClassFile * cf){
    // ve qual o tipo de attribute e libera
...
// libera a memoria que fora alocada para constant_pool
void freeConstantPool(ClassFile * cf){
    ...
    //apos todos CONSTANT_Utf8 serem liberados, libera a constant_pool
    free(cf->constant_pool);
}

void howto()
{
    printf("forma de uso\n%s NomeDaClasse.class t||f\n", HOWTO);
}

int main(int argc, char* argv[])
{
    // pega-se o primeiro argumento de linha de comando como o nome do arquivo .class
    char * class_filename = argv[1];
    
    // se foram inseridos dois argumentos na linha de comando
    if(argc == 3)
    {
        FILE*   class_file;
        FILE*   output;
        
        // selecao de saida em [t]ermial ou em [f]ile
        if (*argv[2] == 't')
        {
           output = stdout;
        }
        ...
        ClassFile *cf = obtainClassFile(class_file);
         ...
         freeClassFile(cf);
        fclose(class_file);
    }else // nao foram inseridos dois argumentos na linha de comando
    {
        howto();
    }
