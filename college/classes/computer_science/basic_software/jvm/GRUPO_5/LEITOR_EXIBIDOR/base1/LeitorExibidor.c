...
void	mostrarClasseDeArquivo(ClasseDeArquivo * ca, FILE * saida){
    fprintf(saida, "\t\t\tLeitor/Exibidor Software Basico 1/2016\n");
...
void	mostrarInfo(ClasseDeArquivo * ca,  FILE * saida){
    ...
    if(ca->CONSTANTEe_pool[ca->classe_arq-1].tag != Classe_const){
        ...
        indicador_nome = ca->CONSTANTEe_pool[ca->classe_arq-1].u.Class.indicador_nome;
        constPool = ca->CONSTANTEe_pool + indicador_nome - 1;
        if(constPool->tag != CONSTANTE_Utf8){
            puts("\nERRO: classe_arq nao referencia um indicador_nome valido.\n");
            exit(EXIT_FAILURE);
        }
        else{
            fprintf(saida, "\t<");
    ...
    if(!ca->super_classe){
        ...
        if(ca->CONSTANTEe_pool[ca->super_classe-1].tag != Classe_const){
            puts("\nERRO: super_classe nao aponta para uma CONSTANTE_Class_info.\n");
            exit(EXIT_FAILURE);
        }
        else{
            fprintf(saida, "Super class: \t\t\tcp_info #%" PRIu16 , ca->super_classe);
            uint16_t	indicador_nome;
            indicador_nome = ca->CONSTANTEe_pool[ca->super_classe-1].u.Class.indicador_nome;
            constPool = ca->CONSTANTEe_pool + indicador_nome - 1;
            if(constPool->tag != CONSTANTE_Utf8){
...
void    ImprimeCONSTANTEeClasse(ClasseDeArquivo * ca, constPoolInf * constPool, FILE * saida)
{
    constPoolInf * cp_a0;
...
void    ImprimeCONSTANTEeString(ClasseDeArquivo * ca, constPoolInf * constPool, FILE * saida)
{
    constPoolInf * cp_a0;
...
void    ImprimeCONSTANTEeNomeTipo(ClasseDeArquivo * ca, constPoolInf * constPool, char descritor_nome, FILE * saida)
{
...
//Imprime no arquivo de saida os valores apontados pelos CONSTANTEe_campo_referencia, CONSTANTEe_metodo_referencia, CONSTANTEe_Interface_referencia
void    ImprimeCONSTANTEeReferencia(ClasseDeArquivo * ca, constPoolInf * constPool,char tipo_indicador, FILE * saida)
{
    constPoolInf * cp_a0;
    constPoolInf * cp_a2;
    switch(tipo_indicador)
    {
        case 'c':
            ...
            {
                puts("\nERRO: valor da tag invalido (Diferente de Classe_const).\n");
                exit(EXIT_FAILURE);
            }
            else
            {
        ...
        case 'n':
            cp_a0 = ca->CONSTANTEe_pool + constPool->u.Ref.indicador_nome_e_tipo - 1;
            if(cp_a0->tag != CONSTANTE_NameAndType)
            {
                puts("\nERRO: valor da tag invalido (Diferente de CONSTANTE_NameAndType).\n");
                exit(EXIT_FAILURE);
            }
            else
            {
                ...
                cp_a2 = ca->CONSTANTEe_pool + cp_a2->u.NameAndType.descriptor_index - 1;
...
void	mostrarConstPool(ClasseDeArquivo * ca, FILE * saida){
    ...
    constPoolInf * constPool;
    uint16_t	i;
    for(constPool = ca->CONSTANTEe_pool, i = 1; constPool < (ca->CONSTANTEe_pool + ca->Cont_ConstPool - 1); constPool++, i++){
        switch(constPool->tag){
            case	Classe_const:
                fprintf(saida, "[%" PRIu16 "]\tCONSTANTE_Class_info\n", i);
                fprintf(saida, "\tNome da Classe:\t\tcp_info #%" PRIu16 "\t<", constPool->u.Class.indicador_nome);
            ...
            case	CONSTANTEe_Interface_referencia:
                ...
                fprintf(saida, "\tNome da classe:\t\tcp_info #%" PRIu16 "\t<", constPool->u.Ref.indicador_nome);
                ImprimeCONSTANTEeReferencia(ca,constPool,'c',saida);
                fprintf(saida, ">\n\tNome e tipo:\t\tcp_info #%" PRIu16 "\t<", constPool->u.Ref.indicador_nome_e_tipo);
                ImprimeCONSTANTEeReferencia(ca,constPool,'n',saida);
                fprintf(saida, ">\n\n");
                break;
            case	CONSTANTE_String:
                fprintf(saida, "[%" PRIu16 "]\tCONSTANTE_String_info\n", i);
                fprintf(saida, "\tString:\t\t\tcp_info #%" PRIu16 "\t<", constPool->u.String.indicador_string);
            ...
            case	CONSTANTE_NameAndType:
                ...
                fprintf(saida, "\tNome:\t\t\tcp_info #%" PRIu16 "\t<", constPool->u.NameAndType.indicador_nome);
...
void	mostrarInterfaces(ClasseDeArquivo * ca, FILE * saida){
    ...
    for(uint16_t i = 0; i < ca->interfaces_count; i++){
        ...
        if(ca->CONSTANTEe_pool[ca->interfaces[i]-1].tag != Classe_const){
            ...
            indicador_nome = ca->CONSTANTEe_pool[ca->interfaces[i]-1].u.Class.indicador_nome;
            constPool = ca->CONSTANTEe_pool + indicador_nome - 1;
            if(constPool->tag != CONSTANTE_Utf8){
...
void	mostrarCampos(ClasseDeArquivo * ca, FILE * saida){
    ...
    for(uint16_t i = 0; i < ca->conts_campos; i++){
        ...
        fprintf(saida,"\tNome:\t\t\tcp_info #%" PRIu16, (ca->fields+i)->indicador_nome);
        fprintf(saida, "\t<");
        ImprimeConstUtf8(constPool, saida);
        fprintf(saida, ">\n");
        ...
        fprintf(saida,"\tDescritor:\t\tcp_info #%" PRIu16, (ca->fields+i)->descriptor_index);
        fprintf(saida, "\t<");
        ImprimeConstUtf8(constPool, saida);
        fprintf(saida, ">\n");
        
        
        fprintf(saida,"\tFlags de Acesso:\t\t0x%.4" PRIx16 "\t\t[", (ca->fields+i)->flags_acesso);
...
void	mostrarMetodos(ClasseDeArquivo * ca, FILE * saida){
    ...
    for(uint16_t i = 0; i < ca->conts_metodo; i++){
        ...
        fprintf(saida,"Nome:\t\t\tcp_info #%" PRIu16, (ca->methods+i)->indicador_nome);
        fprintf(saida, "\t<");
        ImprimeConstUtf8(constPool, saida);
        fprintf(saida, ">\n");
        
        fprintf(saida,"Descritor:\t\tcp_info #%" PRIu16, (ca->methods+i)->descriptor_index);
        ...
        fprintf(saida,"Flags de Acesso:\t\t0x%.4" PRIx16 "\t\t[", (ca->methods+i)->flags_acesso);
...
void	mostrarAtributos(field_info * fd_in, method_info * mt_in, attribute_info * attr_in, ClasseDeArquivo * ca, FILE * saida){
    ...
    for(uint16_t i = 0; i < conts_atributos; i++){
        ...
        switch(getTipoAtributo(attributes_aux, ca)){
            case	CONSTANTE_VALUE:
                ;
                uint16_t	CONSTANTEvalue_index = attributes_aux->u.CONSTANTEValue.CONSTANTEvalue_index;
                fprintf(saida, "\t\tCONSTANTE value index:\tcp_info #%" PRIu16, CONSTANTEvalue_index);
            ...
            case	CODE:
                ...
                for(uint32_t offset = 0; offset < attributes_aux->u.Code.code_length;){
                    ...
                    switch(*nextCode){
                        ...
                        case	new:
                            ...
                            if(cp_a0->tag != Classe_const){
                                printf("\nERRO: %s nao referencia um Classe_const valido.\n",
                                       op_codesJVM[*nextCode]);
                                exit(EXIT_FAILURE);
                            }
                            else{
                                uint16_t	indicador_nome = cp_a0->u.Class.indicador_nome;
                                cp_a0 = ca->CONSTANTEe_pool + indicador_nome - 1;
                        ...
                        case	putstatic:
                            ...
                            fprintf(saida, "\t#%" PRIu16, index);
                            cp_a0 = ca->CONSTANTEe_pool + index - 1;
                            if(cp_a0->tag != CONSTANTEe_metodo_referencia && cp_a0->tag != CONSTANTEe_campo_referencia){
                                printf("\nERRO: %s nao referencia um CONSTANTE_ref valido.\n",
                                       op_codesJVM[*nextCode]);
                                exit(EXIT_FAILURE);
                            }
                            else{
                                uint16_t	indicador_nome = cp_a0->u.Ref.indicador_nome;
                                uint16_t	indicador_nome_e_tipo = cp_a0->u.Ref.indicador_nome_e_tipo;
                                ...
                                if(cp_a0->tag != Classe_const){
                                    ...
                                    cp_a0 = ca->CONSTANTEe_pool + indicador_nome_e_tipo - 1;
                                    if(cp_a0->tag != CONSTANTE_NameAndType){
                                        printf("\nERRO: %s nao referencia um CONSTANTE_ref valido.\n",
                                               op_codesJVM[*nextCode]);
                                        exit(EXIT_FAILURE);
                                        ...
                                        indicador_nome = cp_a0->u.NameAndType.indicador_nome;
                                        cp_a0 = ca->CONSTANTEe_pool + indicador_nome - 1;
                        ...
                        case	iinc: // 1 byte 1 byte
                            ...
                            break;
                            // 3 bytes
                            // CONSTANTEe_pool index (class) (uint16_t) ; unsigned byte (uint8_t)
                        case	multianewarray:
                            fprintf(saida, "%s", op_codesJVM[*nextCode]);
                            index = (uint16_t) (*(nextCode+1) << 8) + *(nextCode+2);
                            fprintf(saida, "\t#%" PRIu16, index);
                        ...
                        case	jsr_w:
                            ...
                            // CONSTANTEe_pool index (uint16_t); unsigned byte (uint8_t); unsigned byte (uint8_t)
                        ...
                        case	invokedynamic:
                            ...
                            fprintf(saida, "\t#%" PRIu16, index);
                            cp_a0 = ca->CONSTANTEe_pool + index - 1;
                            if(cp_a0->tag != CONSTANTEe_Interface_referencia && cp_a0->tag != CONSTANTEe_metodo_referencia){
                                printf("\nERRO: %s nao referencia um CONSTANTE_ref valido.\n",
                                       op_codesJVM[*nextCode]);
                                exit(EXIT_FAILURE);
                            }
                        ...
                        case	lookupswitch:
                            ...
                            for(uint32_t j = 0; j < n; j++){
                                ...
                                offset_label = (*((1+i) + nextCode) << 24) | (*((2+i) + nextCode) << 16) |
                                (*((3+i) + nextCode) << 8) | *((4+i) + nextCode);
                                i += 4;
                                fprintf(saida, ":\t%" PRId32 " (%+" PRId32 ")\n",
                ...
                if(attributes_aux->u.Code.exception_table_length){
                    ...
                    for(uint16_t i = 0; i < attributes_aux->u.Code.exception_table_length; i++){
                        ...
                        constPoolInf	* cp_a0 = ca->CONSTANTEe_pool + (attributes_aux->u.Code.exception_table + i)->catch_type - 1;
            ...
            case	INNER_CLASSES:
                ...
                for(uint16_t i = 0; i < attributes_aux->u.InnerClasses.number_of_classes; i++){
                    ...
                    ImprimeCONSTANTEeClasse(ca, ca->CONSTANTEe_pool +
                    ...
                    uint16_t	flags_acesso = (classes_aux)->inner_class_access_flags;
                    fprintf(saida, "[");
                    if(flags_acesso >= FLAG_ABSTRATA){
                        fprintf(saida, "abstract ");
                        flags_acesso -= FLAG_ABSTRATA;
            ...
            case	LOCAL_VARIABLE_TABLE:
                ...
                for(uint16_t i = 0; i < attributes_aux->u.LocalVariableTable.local_variable_table_length; i++){
                    ...
                    fprintf(saida,"\tcp_info #%" PRId16,
                            ((attributes_aux->u.LocalVariableTable.local_variable_table)+i)->indicador_nome);
                    fprintf(saida,"\tcp_info #%" PRId16 "\n",
                            ((attributes_aux->u.LocalVariableTable.local_variable_table)+i)->descriptor_index);
                    cp_a0 = ca->CONSTANTEe_pool + (((attributes_aux->u.LocalVariableTable.local_variable_table)+i)->indicador_nome) - 1;
                    fprintf(saida,"\t\t\t\t\t\t\t\t");
                    ImprimeConstUtf8(cp_a0, saida);
                    fprintf(saida,"\t");
                    cp_a0 = ca->CONSTANTEe_pool + (((attributes_aux->u.LocalVariableTable.local_variable_table)+i)->descriptor_index) - 1;
                    ImprimeConstUtf8(cp_a0, saida);	
                    fprintf(saida, "\n");
...
// Faz a leitura do arquivo .class, e retorna um ponteiro para estrutura ClasseDeArquivo com os campos preenchidos;
ClasseDeArquivo *	loadClasseDeArquivo(FILE * fi){
	...
	// le uint32_t (4 bytes) do arquivo e atualiza o campo magic do ClasseDeArquivo.
	...
	// le dois uint16_t (2 bytes) do arquivo e atualiza os campos Min_version e Principal_version do ClasseDeArquivo
	...
	// le uint16_t (2 bytes) do arquivo e atualiza o campo flags_acesso do ClasseDeArquivo
...
void		setCONSTANTEPool(ClasseDeArquivo * ca, FILE * fi){
    ca->Cont_ConstPool = read2Byte(fi);  // le o numero de elementos na CONSTANTE pool
    /*	printf("Cont_ConstPool = %" PRIu16, ca->Cont_ConstPool);*/
    if(ca->Cont_ConstPool == 0){
        puts("VerifyError");
        exit(EXIT_FAILURE);
    }
    ca->CONSTANTEe_pool = (constPoolInf *) malloc((ca->Cont_ConstPool -1)*sizeof(constPoolInf)); //FREE OK
    
    constPoolInf * constPool; // declara um tipo const pool para usar e setar
    for(constPool = ca->CONSTANTEe_pool; constPool < (ca->CONSTANTEe_pool + ca->Cont_ConstPool - 1); constPool++){
        constPool->tag = 0;
...
void		setInterfaces(ClasseDeArquivo * ca, FILE * fi){
    ca->interfaces_count = read2Byte(fi); // recebe o numéro direto de superinterfaces dessa classe ou tipos de interface
    if(ca->interfaces_count){
        ...
        ca->interfaces = (uint16_t *) malloc((ca->interfaces_count)*sizeof(uint16_t));	// FREE OK
        for(uint16_t * in = ca->interfaces; in < (ca->interfaces + ca->interfaces_count); in++){
            *in = read2Byte(fi);
        }
    }
    else{
...
void		setFields(ClasseDeArquivo * ca, FILE * fi){
    ca->conts_campos = read2Byte(fi); // recebe o numero de estruturas field_info para alocar e receber informa
    
    if(ca->conts_campos){
        ca->fields = (field_info *) malloc((ca->conts_campos)*sizeof(field_info));	// FREE OK
        for(field_info * fd_in = ca->fields; fd_in < (ca->fields + ca->conts_campos); fd_in++){
...
void		setMethods(ClasseDeArquivo * ca, FILE * fi){
    ca->conts_metodo = read2Byte(fi); // Pega o número de estruturas de method_info
    
    if(ca->conts_metodo){
        // aloca e le as informacoes para o array method_info
        ca->methods = (method_info *) malloc((ca->conts_metodo)*sizeof(method_info));	// FREE OK
        for(method_info * mt_in = ca->methods; mt_in < (ca->methods + ca->conts_metodo); mt_in++){
...
void	setAttributes(field_info * fd_in, method_info * mt_in, attribute_info * attr_in, ClasseDeArquivo * ca,FILE * fi){
    attribute_info *	attributes;
    uint16_t			conts_atributos;
    ...
    if(fd_in != NULL){
        ...
        if(fd_in->conts_atributos){
            fd_in->attributes = (attribute_info *) malloc(fd_in->conts_atributos * sizeof(attribute_info));	
        }
        else{
        ...
        if(mt_in->conts_atributos){
            mt_in->attributes = (attribute_info *) malloc(mt_in->conts_atributos * sizeof(attribute_info));	
        }
        else{
        ...
        if(attr_in->u.Code.conts_atributos){
            attr_in->u.Code.attributes = (attribute_info *) malloc(attr_in->u.Code.conts_atributos * sizeof(attribute_info));
        }
        else{
        ...
        if(ca->conts_atributos){
            ca->attributes = (attribute_info *) malloc(ca->conts_atributos * sizeof(attribute_info));
        }
        else{
            ca->attributes = NULL;
        }
    ...
    for(attribute_info * attr = attributes; attr < (attributes + conts_atributos); attr++){
		...
		else if (attrTypeResult == INNER_CLASSES)
		{
			attr->u.InnerClasses.number_of_classes = read2Byte(fi);
                ...
                for(classes_type * cl = attr->u.InnerClasses.classes;
                    ...
                    cl->saidaer_class_info_index = read2Byte(fi);
		...
		{
			attr->u.LocalVariableTable.local_variable_table_length = read2Byte(fi);
...
void	freeClasseDeArquivo(ClasseDeArquivo * ca){
    ...
    freeMethods(ca);
//Gambiarra 2
    for(attribute_info * attr = ca->attributes; attr < (ca->attributes + ca->conts_atributos); attr++){
...
void	freeCONSTANTEPool(ClasseDeArquivo * ca){
    for(constPoolInf * constPool = ca->CONSTANTEe_pool; constPool < (ca->CONSTANTEe_pool + ca->Cont_ConstPool - 1); constPool++){
		if (constPool->tag == CONSTANTE_Utf8) {
...
void	freeFields(ClasseDeArquivo * ca){
    for(field_info * fd_in = ca->fields; fd_in < (ca->fields + ca->conts_campos); fd_in++){
        for(attribute_info * attr = fd_in->attributes; attr < (fd_in->attributes + fd_in->conts_atributos); attr++){
            freeAttribute(attr, ca);
        }
        if(fd_in->conts_atributos){
...
void	freeMethods(ClasseDeArquivo * ca){
    for(method_info * mt_in = ca->methods; mt_in < (ca->methods + ca->conts_metodo); mt_in++){
        for(attribute_info * attr = mt_in->attributes; attr < (mt_in->attributes + mt_in->conts_atributos); attr++){
            freeAttribute(attr, ca);
        }
        if(mt_in->conts_atributos){
    ...
    switch(getTipoAtributo(attr, ca)){
        ...
        case	CODE:
            ...
            for(attribute_info * attr_ = attr->u.Code.attributes;
                attr_ < (attr->u.Code.attributes + attr->u.Code.conts_atributos); attr_++){
