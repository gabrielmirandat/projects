...
typedef struct attribute_info{
	uint16_t	attribute_name_index;		// aponta para um índice na CONSTANTEe_pool contendo uma CONSTANTE_Utf8 com nome do atributo
...
typedef	struct field_info{
	uint16_t		flags_acesso;			/* máscara de bits que especifica permissões de acesso
							e propriedades do field */
	uint16_t		indicador_nome;			// índice para o CONSTANTEe_pool contendo o nome do field
	uint16_t		descriptor_index;		// índice para o CONSTANTEe_pool contendo um descritor de field válido
...
typedef struct method_info{
	...
	uint16_t		indicador_nome;			/* índice para o CONSTANTEe_pool contendo o nome simples
							do método, ou nome especial <init> */
	uint16_t		descriptor_index;		// índice para o CONSTANTEe_pool contendo um descritor de método válido
...
typedef struct ClasseDeArquivo{
	uint32_t		magic;			// assinatura do arquivo .class = 0xCAFEBABE

	uint16_t		Min_version;		// (m)
	uint16_t		Principal_version;		// (M)	indicam a versao do arquivo no formato M.m
//4 bytes, versão para comparar com o que a JVM pode ler.
	
	uint16_t		Cont_ConstPool;	// número de entradas da tabela CONSTANTEe_pool + 1 
//Cada array de elemento contem um byte que representa o tipo da CONSTANTEe (string, char, int, etc)


	constPoolInf*	CONSTANTEe_pool;		/* tabela de estruturas representando strings, nomes de classes,
	...
	uint16_t*		interfaces;		/* cada entrada desse array é um índice da CONSTANTEe_pool do tipo CONSTANTE_Class_info,
...
void		setAttributes(field_info *, method_info *, attribute_info *, ClasseDeArquivo *, FILE *);
...
void    ImprimeCONSTANTEeReferencia(ClasseDeArquivo *,constPoolInf *, char, FILE *);
...
void	mostrarAtributos(field_info *, method_info *, attribute_info *, ClasseDeArquivo *, FILE *);	// atributos de field, metodo,
