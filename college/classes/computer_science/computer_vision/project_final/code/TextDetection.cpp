...
IplImage * textDetection (IplImage * input,std::string dir, bool dark_on_light)
{
    ...
    // Create Canny Image
    double threshold_low = 300;	//175	//300
    double threshold_high = 600;//320	//600
    IplImage * edgeImage =
            cvCreateImage( cvGetSize (input),IPL_DEPTH_8U, 1 );
    cvCanny(grayImage, edgeImage, threshold_low, threshold_high, 3) ;
    //std::string salva_canny = dir +"(1)"; 
    //cvSaveImage (salva_canny.c_str(), edgeImage);
    ...
    //std::string saida_swt = dir +"(2)";
    //cvSaveImage (saida_swt.c_str(), saveSWT);
    ...
    //std::string saida_comp = dir + "(3)";
    //cvSaveImage (saida_comp.c_str(),output3);
    ...
    for( int row = 0; row < edgeImage->height; row++ ){
        ...
        for ( int col = 0; col < edgeImage->width; col++ ){
            if (*ptr > 0) {
                ...
                float curX = (float)col + 0.5;	//0.5	--pra que?
                float curY = (float)row + 0.5;	//0.5
                int curPixX = col;
                int curPixY = row;
                ...
                while (true) {
                    curX += G_x*prec;	//cria tipo uma reta com a direção dos gradientes
                    curY += G_y*prec;	//
                    if ((int)(floor(curX)) != curPixX || (int)(floor(curY)) != curPixY) {
                        ...
                        if (CV_IMAGE_ELEM ( edgeImage, uchar, curPixY, curPixX) > 0) {
                            ...
                            if (acos(G_x * -G_xt + G_y * -G_yt) < PI/2.0 ) { //era 2.0
				  ...
				  for (std::vector<Point2d>::iterator pit = points.begin(); pit != points.end(); pit++) {
                                    if (CV_IMAGE_ELEM(SWTImage, float, pit->y, pit->x) < 0) {
                                        CV_IMAGE_ELEM(SWTImage, float, pit->y, pit->x) = length;//recebe valor SWT se não tem nenhum
                                    } else {
                                        CV_IMAGE_ELEM(SWTImage, float, pit->y, pit->x) = std::min(length, CV_IMAGE_ELEM(SWTImage, float, pit->y, pit->x));
                                    }								//recebe o menor entre o que já tinha e o SWT atual
				  }
				  r.points = points;
				  rays.push_back(r);
				}
...
void SWTMedianFilter (IplImage * SWTImage,
    ...
    for (std::vector<Ray>::iterator rit = rays.begin(); rit != rays.end(); rit++) {
        ...
        float median = (rit->points[rit->points.size()/12]).SWT;	//2
        
        //RELATORIO
        //Como estamos pegando menos pontos devido à suavização nos algoritmos das bordas, escolhemos um valor menor para os valores SWT por ser 
        //mais consistente, já que diminuímos o numero de direções inconsistentes com aqueles.
        //std::cout << rit->points.size() << std::endl;
	float min = (rit->points[0]).SWT;
	float max = (rit->points[rit->points.size()]).SWT;
        for (std::vector<Point2d>::iterator pit = rit->points.begin(); pit != rit->points.end(); pit++) {
            CV_IMAGE_ELEM(SWTImage, float, pit->y, pit->x) = std::min(pit->SWT, median); //median
        }
    }
}
            ...
            for (int col = 0; col < SWTImage->width; col++ ){
                if (*ptr > 0) {
                    ...
                    if (col+1 < SWTImage->width) {
                        float right = CV_IMAGE_ELEM(SWTImage, float, row, col+1);	//111111111111111111111111111111111111111111111
                        if (right > 0 && ((*ptr)/right <= 3.0 || right/(*ptr) <= 3.0))	//pixels na CC se razão entre SWs entre 1/3 a 3
                      ...
                      std::vector<std::pair<Point2d,Point2d> > & compBB )
{
       ...
       for (unsigned int i = 0; i < validComponents.size(); i++) {	//222222222222222222222222222222222222
            int count = 0;						//Cada CC não inclui mais de duas outras CCs
            ...
            if (count < 2) { //2						//222222222222222222222222222222222222
                tempComp.push_back(validComponents[i]);			//Cada CC não inclui mais de duas outras CCs
...
std::vector<Chain> makeChains( IplImage * colorImage,
    ...
    for ( unsigned int i = 0; i < components.size(); i++ ) {
        for ( unsigned int j = i + 1; j < components.size(); j++ ) {
            // TODO add color metric										//333333333333333333333333
            if ( (compMedians[i]/compMedians[j] <= 2.0 || compMedians[j]/compMedians[i] <= 2.0) &&		//Lógica união em chains
    ...
    std::cerr << std::endl;
    const float strictness = PI/3.0;	//strictness = rigor
    //merge chains
    int merges = 1;
    while (merges > 0) {
        ...
        for (unsigned int i = 0; i < chains.size(); i++) {
            for (unsigned int j = 0; j < chains.size(); j++) {				//444444444444444444444444444444
                if (i != j) {								//Lógica de união entre chains
...
/*
    ...
    for (std::vector<Chain>::iterator cit = chains.begin(); cit != chains.end(); cit++) {
        if (cit->components.size() >= 2) {	//se a chain tem pelo menos 3 components
