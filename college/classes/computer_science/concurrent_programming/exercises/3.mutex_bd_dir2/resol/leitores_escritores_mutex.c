...
void* reader(void *arg) {
	...
	{               					   					/* repete para sempre */
		 ...
		 pthread_mutex_lock(&mutex);       					/* obtém acesso exclusivo à 'rc' */
		 rc = rc + 1;            		   					/* um leitor a mais agora */
		 if (rc == 1)  pthread_mutex_lock(&db);             /* se este for o primeiro leitor... */
		 pthread_mutex_unlock(&mutex);       				/* libera o acesso exclusivo a 'rc' */
	     pthread_mutex_unlock(&turno); 
		 
		 read_data_base(i);       						    /* acesso aos dados */
		 
		 pthread_mutex_lock(&mutex);       					/* obtém acesso exclusivo a 'rc' */
		 rc = rc - 1;            							/* um leitor a menos agora */
		 if (rc == 0)  pthread_mutex_unlock(&db);       	/* se este for o último leitor */   
		 pthread_mutex_unlock(&mutex);       			    /* libera o acesso exclusivo a 'rc' */
