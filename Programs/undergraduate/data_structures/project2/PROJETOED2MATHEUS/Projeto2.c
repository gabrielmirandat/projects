...
Voo* ins_fim (Voo* fim, char nome[], char tipo, int comb) //funcao auxiliar de incersao
{
...
void insere (FilaVoos* f, char nome[], char tipo, int comb) //funcao de incersao
{
...
int geraRand(int min, int max) //funca de gerar numeros aleatorios
{
    int r;
...
void imprime(FilaVoos* f) //funcao de imprimir filas
{
    Voo* q;
...
Voo* ret_ini (Voo* ini) //funcao auxiliar de remocao
{
    Voo* p = ini->prox;
    free(ini);
    return p;
...
void retira (FilaVoos* f) //funcao de remocao
{
    if (vazia(f))
    {
        printf("Fila vazia.\n");
...
int main()
{
    ...
    int nro_aprox = geraRand(0, 26); //gera numeros aleatorios para os voos
    int nro_decol = geraRand(0, nro_aprox/1.625); //nro_aprox/1.625 usado para manter a proporcao de 26/16 (n de aproximacoes e decolagens maximos permitido)
    int nro_voos = nro_aprox+nro_decol;

    int i, j, *vetcomb, aux, tempo[2]= {0,0}, stat_p1=0, stat_p2=0, stat_p3=0, acabou=0;
    char tipo;
    ...
    j = i;
    tipo = 'D';
    for(i=i-1; i<nro_decol+j-1; i++)  //coloca voos que estao decolando na fila da pista 3
    {
    ...
    while (acabou==0) //roda o programa enquanto houver tempo ou avioes
    {
        ...
        tempo[0] = tempo[0]+5; //adianta 1 ciclo
